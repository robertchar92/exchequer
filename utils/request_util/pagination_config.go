...
const (
	...
	ExactStringType string = "EXACT_STRING"
...
func buildOrder(conditions map[string][]string) string {
	...
	if len(conditions["sort"]) > 0 {
		...
		for _, elem := range conditions["sort"] {
			subValues := strings.Split(elem, ",")
			for _, subValue := range subValues {
				values := strings.Split(subValue, " ")
				if !(len(values) == 2 && (strings.ToUpper(values[1]) == "ASC" || strings.ToUpper(values[1]) == "DESC")) {
					continue
				}
				filteredConditions = append(filteredConditions, subValue)
			}
		}
		orders = strings.Join(filteredConditions, ",")
		...
		if len(conditions[name]) > 0 {
			...
			case StringType:
				for _, value := range conditions[name] {
					scopes = append(scopes, scope.WhereLikeScope(name, value))
				}
			case BoolType:
				boolean := false
			...
			case DatetimeType:
				minmax := strings.Split(conditions[name][0], ",")
				min, _ := now.Parse(minmax[0])
				if !helpers.CheckContainsTime(minmax[0]) {
					min = now.New(min).BeginningOfDay()
				}

				max, _ := now.Parse(minmax[1])
				if !helpers.CheckContainsTime(minmax[1]) {
